...
router.get('/:collection/:parentresource/:childresource/:grandchildresource/:grandchildid',function(req,res){
	...
	var grandchildid = req.params.grandchildid;
...
//validating the requested collection
		...
		//querying the database
		var childquery = {};
		var key = childresource + "." + "person" + "." + grandchildresource  ;
		childquery[key]=grandchildid;
		parentid["name"] = parentresource;
		console.log(childquery);
		collections.find({$and:[childquery,parentid]},'relationships.$:1',function(err,docs){
		//sending the response to the client
		if(err){
			console.log(err);
		}
